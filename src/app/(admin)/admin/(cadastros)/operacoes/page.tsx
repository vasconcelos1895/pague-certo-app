...
import { type OperationFormValues } from "@/lib/validators/operation";
...
const defaultRecordValues: OperationFormValues = {
  name: "",
  description: "",
};
...
export default function Page() {
  ...
  const createRecord = api.operation.create.useMutation({
    async onMutate(newRecord) {
      await utils.operation.list.cancel();
      const prevData = utils.operation.list.getData();
      utils.operation.list.setData(undefined, (old) => [
        ...(old ?? []),
    ...
    onError: (err, _newRecord, ctx) => {
      if (ctx?.prevData) utils.operation.list.setData(undefined, ctx.prevData);
      toast.error(`Erro ao criar registro: ${err.message}`);
    },
    onSuccess: () => {
      toast.success("Registro criado com sucesso!");
    },
    onSettled: () => utils.operation.list.invalidate(),
  });

  // ✅ Optimistic Update + Toast
  const updateRecord = api.operation.update.useMutation({
  ...
  const deleteRecord = api.operation.delete.useMutation({
    async onMutate({ id }) {
      await utils.operation.list.cancel();
      const prevData = utils.operation.list.getData();
      utils.operation.list.setData(undefined, (old) =>
  ...
  const [editingRecord, setEditingRecord] = useState<Operation | null>(null);
