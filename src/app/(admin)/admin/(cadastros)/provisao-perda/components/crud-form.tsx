...
import { type ProvisionForIncurredLosseFormValues, provisionForIncurredLosseSchema } from "@/lib/validators/provisionForIncurredLosse";
import FormInputNumber from "@/components/FormInputNumber";

interface CrudFormProps {
  defaultValues?: Partial<ProvisionForIncurredLosseFormValues>;
  onSubmit: (values: ProvisionForIncurredLosseFormValues) => void;
  isLoading?: boolean;
}

export function CrudForm({ defaultValues, onSubmit, isLoading }: CrudFormProps) {
  const form = useForm<ProvisionForIncurredLosseFormValues>({
    resolver: zodResolver(provisionForIncurredLosseSchema),
    defaultValues,
  });
  ...
  return (
    <FormProvider {...form}>
      ...
      >
        <FormInput name="criteria" label="Critério" control={form.control} />
