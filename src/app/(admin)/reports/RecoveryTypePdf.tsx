...
export function RecoveryTypePdf({ recoveryType = [], title = "Relatório" }: Props) {
  return (
    <Document>
      <Page size="A4" style={styles.page} orientation="landscape">
        ...
        <View style={styles.headerContainer} fixed>
          ...
          <Text style={styles.headerMeta}>
            Gerado em: {new Date().toLocaleString("pt-BR")} — Total de registros: {recoveryType.length}
          </Text>
        </View>

        {/* Cabeçalho da tabela */}
        <View style={styles.tableHeader} fixed>
          <Text style={[styles.tableCell, { width: "35%" }]}>Nome</Text>    
          <Text style={[styles.tableCell, { width: "65%" }]}>Descrição</Text>
        </View>

 

        {/* Renderiza linhas do órgão */}
        {recoveryType.map((n, i) => (
          <View key={(n as any).id ?? i} style={styles.tableRow}>
            <Text style={[styles.tableCell, { width: "35%" }]}>{n.name ?? " - "}</Text>                                               
            <Text style={[styles.tableCell, { width: "65%" }]}>{n.description ?? " - "}</Text>
          </View>
        ))}
