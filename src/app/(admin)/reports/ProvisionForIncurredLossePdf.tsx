...
interface Props {
  provisionForIncurredLosse: ProvisionForIncurredLosse[];
  title?: string;
}

export function ProvisionForIncurredLossePdf({ provisionForIncurredLosse = [], title = "Relatório" }: Props) {
  return (
    <Document>
      <Page size="A4" style={styles.page} orientation="landscape">
        ...
        <View style={styles.headerContainer} fixed>
          ...
          <Text style={styles.headerMeta}>
            Gerado em: {new Date().toLocaleString("pt-BR")} — Total de registros: {provisionForIncurredLosse.length}
          </Text>
        </View>

        {/* Cabeçalho da tabela */}
        <View style={styles.tableHeader} fixed>
          <Text style={[styles.tableCell, { width: "26%" }]}>Critério</Text>
          <Text style={[styles.tableCell, { width: "17%" }]}>Prazo inicial (meses)</Text>
          <Text style={[styles.tableCell, { width: "17%" }]}>Prazo Final</Text>
        ...
        {provisionForIncurredLosse.map((n, i) => (
          <View key={(n as any).id ?? i} style={styles.tableRow}>
            <Text style={[styles.tableCell, { width: "26%" }]}>{n.criteria ?? " - "}</Text>
            <Text style={[styles.tableCell, { width: "17%" }]}>{n.initialDeadline ?? " - "}</Text>
            <Text style={[styles.tableCell, { width: "17%" }]}>{n.finalDeadline?.toLocaleString("pt-BR", { minimumFractionDigits: 2 }) ?? " - "}</Text>
