...
import { RecoveryTypePdf }  from "@/app/(admin)/reports/RecoveryTypePdf"
...
export async function POST(req: Request) {
  try {
    ...
    const recoveryType = await req.json();


    // Validação simples
    if (!recoveryType || !Array.isArray(recoveryType)) {
      return NextResponse.json({ error: "Dados inválidos" }, { status: 400 });
    }

    if (!RecoveryTypePdf || typeof RecoveryTypePdf !== "function") {
      console.error("RecoveryTypePdf inválido:", RecoveryTypePdf);
      return NextResponse.json({ error: "RecoveryTypePdf inválido" }, { status: 500 });
    }

    // renderToStream retorna Node Readable
    const nodeStream = (await renderToStream(
      React.createElement(RecoveryTypePdf, {
        recoveryType,
    ...
    return new NextResponse(webStream, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="rel-recoveryType.pdf"`,
      },
    });
  } catch (err: any) {
