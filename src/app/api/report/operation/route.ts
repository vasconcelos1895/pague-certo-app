...
import { OperationPdf }  from "@/app/(admin)/reports/OperationPdf"
...
export async function POST(req: Request) {
  try {
    ...
    if (!OperationPdf || typeof OperationPdf !== "function") {
      console.error("OperationPdf inválido:", OperationPdf);
      return NextResponse.json({ error: "OperationPdf inválido" }, { status: 500 });
    }

    // renderToStream retorna Node Readable
    const nodeStream = (await renderToStream(
      React.createElement(OperationPdf, {
        operation,
    ...
    return new NextResponse(webStream, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="rel-operation.pdf"`,
      },
    });
  } catch (err: any) {
